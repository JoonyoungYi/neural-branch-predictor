...
class Perceptron:
    ...
    def predict(self, global_branch_history):
        ...
        for i in range(
                0, self.N):  # dot product of branch history with the weights
    ...
    def update(self, prediction, actual, global_branch_history, running_sum):
        if (prediction != actual) or (abs(running_sum) < self.threshold):
            ...
            for i in range(0, self.N):
                self.weights[
                    i] = self.weights[i] + (actual * global_branch_history[i])

    def statistics(self):
        print ("bias is: " + str(self.bias) + " weights are: " + str(
            self.weights))
...
def perceptron_pred(trace, l=1):
    ...
    for br in trace:  # iterating through each branch
        ...
        actual_value = 1 if br[1] else -1
        p_list[br[0]].update(pr, actual_value, global_branch_history,
                             running_sum)
